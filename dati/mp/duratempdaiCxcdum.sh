...
grep -v "9999" /cygdrive/d/grassworks/gb/script/EstrazioneDati2/OUT/${ID}_R.txt | tail -1 | awk '{ printf "%d %-s %-s %.2f\n", $1, $2, $3, $4/100 }' | sed 's/:00.000//g' > ${d}livd_${nome}.txt
else
...
#dum=`awk '{print $2}' ${d}mp123_$nome_a.txt | sed 's/\// /g' | awk '{print $3$2$1}' | sort | tail -1`
#echo $dum
