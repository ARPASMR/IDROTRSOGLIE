...
grep -v "9999" /cygdrive/d/grassworks/gb/script/EstrazioneDati2/OUT/${ID}_R.txt |grep -v "99900" | tail -1 | awk '{ printf "%d %-s %-s %.2f\n", $1, $2, $3, $4/100 }' | sed 's/:00.000//g' > ${d}livd_${nome}.txt
else
...
awk '{ if ( $3 == "2num" ) print $2,$14; else print $2,$9 }' ${d}param_sc.sh > ${d}sitimis.tmp
grep -e "$nome" ${d}sitimis.tmp | awk '{ print $2 }' > ${d}dm.tmp
if [ -s ${d}dm.tmp ]
then
read dataminima < ${d}dm.tmp
grep -e "$nome" ${d}gaugings.txt | awk '{if ($13 < 150) print $1,$4,$10,$11,$16}' | grep "201[345]" | sed -e 's/\// /g'| awk '{print $1,$4$3$2, $2"/"$3"/"$4, $5,$6,$7}' | awk -v a=$dataminima '{if ( $2 >= a ) print $1,$3,$4,$5,$6 }' > ${d}mp123_$nome.txt
...
grep -v "Consorzi" ${d}mp123_$nome.txt > ${d}mp123_$nome_a.txt


awk '{print $2}' ${d}mp123_$nome_a.txt | sed 's/\// /g' > ${d}dum1.tmp
awk '{print $3$2$1}' ${d}dum1.tmp | sort > ${d}dum2.tmp
tail -1 ${d}dum2.tmp > ${d}dum.tmp
read dum < ${d}dum.tmp
awk '{print $3}' ${d}mp123_$nome_a.txt > ${d}mpl_$nome.txt
...
awk -v l=$liv '{print $2-l, $3}' ${d}livm_$nome_xc.txt | awk '{for (i=1; i<=NF; i++) if ($i < 0) $i = -$i; print }'| sort | sed -n -e 1p > ${d}mplp_$nome.txt
...
cat ${d}grassindum.sh | /usr/local/bin/grass62 -text /cygdrive/d/grassworks/gb/MISURE/
rm  ${d}gisinput_*.txt
