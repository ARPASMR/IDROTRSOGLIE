...
wc -l ${d}mp_siti3.txt > ${d}amp.tmp
read nn fuffa < ${d}amp.tmp
for ((t=1; t<=nn; t++))
do
head -$t ${d}mp_siti3.txt | tail -1 > ${d}bmp.tmp
...
paste ${d}val.tmp ${d}l.tmp | awk '{print $2, $1, $4, $6}' > ${d}livm$x.tmp
cat ${d}livm*.tmp > ${d}livm_$nome.txt
done
...
cp ${d}mp${nome}.tmp ${d}cdate/mp${nome}.tmp
		...
		set output \"/cygdrive/d/grassworks/scambio/wl${ID}mp.png\"
		...
		plot \"${d}durate/cdur_${nome}.txt\" using 2:1 with lines lt 3 lw 3 axes x1y2, \
		 ...
		 \"${d}livf_${nome}.txt\" u 3:2:2 with labels axes x1y2, \
